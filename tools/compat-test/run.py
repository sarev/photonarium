...
def _run_cmd(cmd: list[str], label: str, cwd: str | None = None, suppress_stderr: bool = False) -> tuple[bool, str]:
    """
...
def run_combo(combo_idx: int, combo: dict, python_exe: str) -> dict:
    ...
    """
    label = f'Combo {combo_idx}: Python {combo["python"]}, torch={combo["torch"]}, transformers={combo["transformers"]}'
    print(f'\n{"=" * 70}')
    print(f'  {label}')
    print(f'  ({combo["note"]})')
    print(f'{"=" * 70}')
    ...
    try:
        ...
        # Step 1: Create venv
        ok, output = _run_cmd([python_exe, '-m', 'venv', str(venv_dir)], 'Creating venv')
        ...
        # Step 2: Upgrade pip
        ok, output = _run_cmd([venv_python, '-m', 'pip', 'install', '--upgrade', 'pip'], 'Upgrading pip')
        ...
        ok, output = _run_cmd([venv_pip, 'install', 'open_clip_torch'], 'Installing OpenCLIP')
        ...
        ok, output = _run_cmd(
            [venv_pip, 'install'] + deps, f'Installing remaining deps (transformers={combo["transformers"]})'
        )
        ...
        try:
            ...
            if smoke_result.returncode == 0:
                ...
                all_ok = all(v['ok'] for v in smoke_data.get('imports', {}).values())
                ...
                result['smoke_test'] = {'error': smoke_result.stderr[:500] or smoke_result.stdout[:500]}
        except subprocess.TimeoutExpired:
            print('TIMEOUT')
        ...
        if venv_dir.exists():
            print('    Cleaning up venv...', end=' ', flush=True)
            shutil.rmtree(venv_dir, ignore_errors=True)
            print('done')
...
def _print_summary(results: list[dict]) -> None:
    ...
    if passes:
        ...
        for r in passes:
            ...
            versions = {pkg: info.get('version', '?') for pkg, info in imports.items() if info.get('ok')}
            cuda = smoke.get('cuda', {})
...
def main() -> None:
    ...
    if args.list:
        ...
        for i, combo in enumerate(COMBOS, 1):
            print(f'{i:>3}  {combo["python"]:>6}  {combo["torch"]:>6}  {combo["transformers"]:>13}  {combo["note"]}')
        print()
        return
    ...
    with open(RESULTS_FILE, 'w', encoding='utf-8') as f:
        f.write('Photonarium Compatibility Test Results\n')
        f.write(f'{"=" * 50}\n')
