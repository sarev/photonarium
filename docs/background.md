...
| **Non-destructive editing** | Basic | Yes | Basic | Yes | No | No | Yes | Yes | No | No | No | No |
...
The trade-offs are: no multi-user support and no manual photo editor - there are no tone curves, cropping, or masking tools, though Photonarium does offer one-click neural enhancements (denoise, deblur, sharpen, and upscale) that always save a new version rather than altering your original. Photonarium also has no built-in phone backup - but this is arguably a feature, not a gap. If you run Photonarium on a NAS, you can use whatever sync tools you already have (Synology Cloud Sync, QNAP HybridMount, or native apps for Apple Photos, Google Photos, OneDrive, Dropbox, etc.) to get photos and videos onto the NAS, and Photonarium simply indexes them. No need to replace a workflow that already works. The web UI is fully responsive, so browsing your library from a phone or tablet works well - you just don't need a dedicated app.
