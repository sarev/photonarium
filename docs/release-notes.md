...
- **Face recognition "snowball" after manual tagging**: the per-person reassessment sweep that fires after each manual tag used *all* of that person's faces as reference — including faces auto-assigned seconds earlier by the previous sweep in the same session. Tagging a handful of faces could chain outward through its own conquests and mis-assign hundreds of unrelated faces to that person (observed in practice: 5 tags → 361 assignments, most under 0.3 similarity to any user-confirmed face). Person-scoped sweeps now use manually-tagged reference faces only — the same anti-snowball rule the full sweep has always had. Mis-assigned faces from earlier sessions are unwound automatically by the next full sweep.
- **Inconsistent matching between reassessment paths**: the background (async) reassessment matched each face to its single highest-similarity known face, while the pipeline's synchronous sweep prefers named people over the ignored pile ('-') and honours per-person thresholds properly. The same library could churn as the two paths overrode each other's decisions. Both now share one matching implementation (named-before-ignored, per-person thresholds).
- **Misleading "no matches" log diagnostic**: the reassessment summary reported a matrix-wide best similarity (e.g. `best similarity: 0.993`) even when that value came from a face already correctly assigned, making healthy runs look like missed matches. The summary now reports how many candidates were already correctly assigned and the closest similarity among candidates that genuinely matched nobody.
