        ...
        autoUpsert(people) {
            ...
            transaction(() => {
                for (const p of people) {
                    ...
                    if (existing) {
                        // Preserve locally-computed face_count — the backend
                        // snapshot may be stale (e.g. person created before
                        // faces were assigned).  face_count is a derived
                        // value maintained by reconcilePerson(), not an
                        // authoritative backend field.
                        const { face_count: _ignored, ...fields } = p;
