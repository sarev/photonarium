    ...
    (
        ...
        [
            ...
            (
                'catalogue_dir',
                [
                    'Where imported photos are stored, organised by date.',
                    'Used when importing from cameras, phones, or downloads via the UI.',
            ...
            (
                'server_host',
                [
                    '[!] Which network to listen on.',
                    '  0.0.0.0   = all networks (other devices on your Wi-Fi can connect)',
            ...
            (
                'headless',
                [
                    'Enable this if Photonarium is running on a server without a desktop',
                    '(e.g. Docker, NAS, or a remote machine). Hides the "Add Local Folder"',
                    'button and disables clickable file paths.',
                ],
            ),
            (
    ...
    (
        ...
        [
            (
                ...
                [
                    'How many images to process at once for search indexing (1-256).',
                    'Larger values are faster but need more GPU memory.',
                    'If you see out-of-memory errors, try lowering this.',
            ...
            (
                'nima_batch_size',
                [
                    'How many images to score at once for quality ranking (1-256).',
                    'Larger values are faster but need more GPU memory.',
                    'Run tools/benchmark_batch_sizes.py to find the best value for your hardware.',
                ],
            ),
            ...
            (
                ...
                [
                    'When only a few photos change, duplicates are checked incrementally.',
                    'If more than this many photos need checking, a full rebuild is done',
                    'instead (which is faster for large numbers). Range: 1-10000',
                ],
            ),
            (
                'incremental_threshold_percent',
                [
                    'If the number of changed photos exceeds this percentage of your total',
                    'library, duplicate detection does a full rebuild instead of checking',
                    'each one individually. Range: 5-50, recommended: 15-25',
                ],
            ),
        ],
    ),
        ...
        [
            (
                ...
                [
                    ...
                    '',
                    'Options:',
                    '  - ViT-B-32  (fast, good quality, ~400 MB)',
                    '  - ViT-B-16  (slower, better quality, ~550 MB)',
                    '  - ViT-L-14  (slowest, best quality, ~1.8 GB)',
                    '',
                    'For non-English text search, use a multilingual model instead:',
                    '  - nllb-clip-base-siglip  (200+ languages, ~600 MB)',
                    '  - nllb-clip-large-siglip (higher quality, ~1.2 GB)',
                    '',
                    'Changing this model will re-process all your photos (may take a while',
                    'for large libraries) and requires downloading the new model first.',
                ],
            ),
            (
                'openclip_pretrained',
                [
                    '[M] Training data used for the search model. Must be compatible with',
                    'the model chosen above. Common options:',
                    '  - openai            (the original CLIP training data)',
            ...
            (
                'caption_model',
                [
                    '[M] The AI model that generates text descriptions of your photos.',
                    'Larger models write better captions but are slower and need more memory.',
                    '',
                    'Options:',
                    '  - Salesforce/blip-image-captioning-base   (~1 GB, fast)',
                    '  - Salesforce/blip-image-captioning-large  (~2 GB, better quality)',
                    '  - Salesforce/blip2-opt-2.7b               (~5 GB, best quality)',
                    '  - Salesforce/blip2-flan-t5-xl             (~8 GB, most descriptive)',
                    '',
                    'All caption models generate English-only text.',
                ],
            ),
            (
                'stt_model',
                [
                    '[M] The Whisper model used to transcribe speech in videos.',
                    'Larger models are more accurate but slower and need more memory.',
                    '',
                    '  tiny     - fastest, best for English only (~75 MB)',
                    '  base     - good balance for English (~140 MB)',
                    '  small    - good for multiple languages (~460 MB)',
                    '  medium   - high accuracy, strong multilingual (~1.5 GB)',
                    '  large-v3 - best accuracy, recommended for non-English (~3 GB)',
                ],
            ),
        ],
    ),
    ...
    (
        ...
        [
            (
                'image_extensions',
                [
                    'File types recognised as photos (lowercase, with leading dot).',
                    'Add or remove entries to control which files Photonarium indexes.',
                ],
            ),
            (
                'video_extensions',
                [
                    'File types recognised as videos (lowercase, with leading dot).',
                ],
            ),
            (
                'max_image_dimension',
                [
                    'Photos larger than this (in pixels, width or height) are scaled down',
                    'before processing. Prevents memory issues with huge panoramas.',
                    'Set to 0 to process at full size (not recommended for very large images).',
                    'Range: 0 or 1024-65536',
                ],
            ),
            (
                'thumbnail_quality',
                [
                    ...
                    'Higher values look sharper but take more disk space.',
                ],
            ),
            (
                'date_order',
                [
                    'How to interpret ambiguous dates in filenames.',
                    'For example, "07-03-2024" could be 7 March or 3 July.',
                    'This only matters when the date format is genuinely ambiguous —',
                    'if the preferred reading would be invalid (e.g. month 13),',
                    'the valid alternative is used automatically.',
            ...
            (
                ...
                [
                    'Some apps (like WhatsApp) set the wrong date in photo metadata but',
                    'encode the real date in the filename. Add filename patterns here to',
                    'tell Photonarium to trust the filename date instead.',
                    'One pattern per line, using * as a wildcard (e.g. "WhatsApp Image *").',
                ],
            ),
        ],
    ),
    ...
    (
        ...
        [
            (
                ...
                [
                    'Automatically find faces in your photos during scanning.',
                    'When disabled, face-related features are hidden in the UI.',
                ],
            ),
            (
                'face_detection_min_confidence',
                [
                    'How confident the detector must be before reporting a face.',
                    'Higher values mean fewer false detections but may miss some faces.',
            ...
            (
                ...
                [
                    'Ignore faces smaller than this many pixels (width or height).',
                    'Helps filter out tiny, unrecognisable faces in crowd shots.',
                    'Range: 20-200 pixels, recommended: 40-80',
                ],
            ),
            (
    ...
    (
        ...
        [
            (
                ...
                [
                    'Sensitivity for detecting scene changes in videos.',
                    'Lower values detect more subtle cuts; higher values only detect obvious ones.',
                    'Range: 1-100, recommended: 20-35',
                ],
            ),
            (
                'video_max_scene_duration',
                [
                    'Maximum length of a single scene (in seconds).',
                    'Longer scenes are split into chunks. Also used as the fallback',
                    'when no scene changes are detected in a video.',
                ],
            ),
            (
                'stt_enabled',
                [
                    'Transcribe speech in videos so you can search by what was said.',
                    'Requires the faster-whisper package.',
                    'When disabled, videos are still indexed visually — only speech',
            ...
            (
                'stt_language',
                [
                    'Language for transcription (e.g. "en", "fr", "de").',
                    'Leave empty to detect the language automatically.',
                    'Auto-detection works best with the "small" model or larger.',
                ],
            ),
            (
                'stt_languages',
    ...
    (
        ...
        [
            (
                ...
                [
                    'How different two photos can look and still count as "near-identical".',
                    'Lower values are stricter (only very close matches).',
                    'Range: 0-64, recommended: 4-8',
                ],
            ),
            (
                'similarity_threshold_level2',
                [
                    'How similar two photos must be to count as "similar" (e.g. same scene,',
                    'different crop or exposure). Higher values are stricter.',
                    'Range: 0.0-1.0, recommended: 0.93-0.97',
                ],
            ),
            (
                'similarity_threshold_level3',
                [
                    'How similar two photos must be to count as "related" (e.g. same',
                    'subject, different angle). Higher values are stricter.',
                    'Range: 0.0-1.0, recommended: 0.80-0.90',
                ],
            ),
        ],
    ),
    ...
    (
        ...
        [
            (
                'nima_enabled',
                [
                    'Enable a second AI quality scorer (NIMA) alongside the built-in one.',
                    'When both are active, their scores are blended for better accuracy',
                    'in the Quality sort and when picking the best photo in a duplicate group.',
                    '',
                    'Turning this off saves some processing time and GPU memory.',
                    'Existing scores are kept — only new photos skip NIMA scoring.',
                ],
            ),
            (
                'quality_weight_aesthetic',
                [
                    'How much each factor matters when ranking photos by quality.',
                    'These only affect the sort order — your photos are not changed.',
                    'The four weights should add up to roughly 1.0.',
                    '',
                    '  aesthetic - how visually appealing the photo is (AI score)',
                    '  sharpness - how sharp/in-focus the photo is',
                    '  pixels    - resolution (larger photos rank higher)',
                    '  bpp       - detail density (bits per pixel)',
                ],
            ),
            ...
            (
                'quality_alpha',
                [
                    'Balance between the two quality scorers (NIMA and LAION).',
                    '0.0 = use only LAION, 1.0 = use only NIMA, 0.6 = mostly NIMA.',
                    'Only matters when NIMA is enabled. Range: 0.0-1.0',
                ],
            ),
        ],
    ),
    ...
    (
        ...
        [
            (
                'on_this_day_enabled',
                [
                    'Show an "On this day..." album of past photos when you open the app',
                    'after being away for a while (8+ hours). Only appears if you have',
            ...
            (
                'slideshow_interval',
                [
                    'How long each photo is shown during a slideshow (in seconds).',
                    'Range: 1.0-60.0',
                ],
            ),
        ],
    ...
    (
        ...
        [
            (
                ...
                [
                    'How many thumbnails the browser loads at once.',
                    'More = faster loading, but heavier on the server.',
            ...
            (
                ...
                [
                    'Give up loading a thumbnail after this many milliseconds.',
            ...
            (
                ...
                [
                    'How often to recalculate which thumbnails to load while scrolling.',
