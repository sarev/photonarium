...
- **Video transcriptions** - automatic speech-to-text for video audio, enabled by default with automatic language detection. Transcription text is displayed as subtitles during playback, shown in scene preview popups, and is semantically searchable. Each video's language can be set individually for accurate transcription of multilingual libraries.
